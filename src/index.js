...
const { blockBadWords } = require("./v1/middlewares/sensor.middleware");
...
app.use(
	express.urlencoded({ extended: true, charset: "utf8", limit: "500mb" }),
);
app.use(cookieParser());
app.use(blockBadWords);
...
const server = app.listen(port, async () => {
	...
	terminal.info(
		`index.js | ORS_ORIGIN loaded as: ${process.env.CORS_ORIGIN}`,
	);
	terminal.info(`index.js | NODE_ENV loaded as: ${process.env.NODE_ENV}`);
	terminal.info(
		`index.js | CORS_HTTP_ONLY loaded as: ${process.env.CORS_HTTP_ONLY}`,
	);

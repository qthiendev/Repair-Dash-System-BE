...
module.exports = async (trans_id) => {
    ...
    if (!payment) {
        terminal.warning(`check.zaloplay.service.js | No payment record found for transaction {appTransID:${trans_id}}`);
        return -1;
    }
    ...
    const response = await axios.post(config.endpoint_query, qs.stringify(postData), {
        headers: {
    ...
    if (!response) {
        terminal.error(`check.zaloplay.service.js | Cannot get respone of ${config.endpoint_query} => {appTransID:${trans_id}}`);
        return -2;
    }
    ...
    if (response.data.return_code === 2) {
        ...
        if (!updatePayment) {
            terminal.warning(`check.zaloplay.service.js | Cannot update CANCELED for transaction #${trans_id}}`);
            return -3;
        }
    ...
    if (response.data.return_code === 1) {
        ...
        if (!updatePayment) {
            terminal.warning(`check.zaloplay.service.js | Cannot update COMPLETED for transaction #${trans_id}}`);
            return -3;
        }
        ...
        if (!updateUser) {
            terminal.warning(`check.zaloplay.service.js | Cannot update user_priority for user_id #${payment.user_id}}`);
            return -3;
        }

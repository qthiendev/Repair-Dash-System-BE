...
module.exports = async (user_id, trans_id) => {
    if (!trans_id) {
        const payments = await Payment.findAll({ attributes: { exclude: ['delete_flag'] }, });

        if (!payments) {
            terminal.warning(`getSubscription.zalopay.service.js | No payment found in system.`);
            return -1;
        }
    ...
    if (!payment) {
        terminal.warning(`getSubscription.zalopay.service.js | No payment found for user_id: ${user_id}, trans_id: ${trans_id}`);
        return -1;
    }

...
const isValidService = async (service_id) => {
	const service = await Service.findOne({
		where: { service_id, delete_flag: false },
	});
	return !!service;
};
...
module.exports = async (customer_id, store_id = null, service_id = null) => {
	if (!(await isValidCustomer(customer_id))) {
		terminal.warning(
			`createFavorite.service.js | Customer ${customer_id} not found.`,
		);
		return -1;
	}
	...
	if ((store_id && service_id) || (!store_id && !service_id)) {
		terminal.warning(
			`createFavorite.service.js | Must provide either store_id or service_id, but not both.`,
		);
		return -2;
	}
	...
	if (store_id) {
		...
		const existing = await Favorite.findOne({
			where: { customer_id, store_id },
		});
		if (existing) {
			terminal.warning(
				`createFavorite.service.js | Favorite already exists for customer ${customer_id} and store ${store_id}.`,
			);
			return -6;
		}
	}
	...
	if (service_id) {
		if (!(await isValidService(service_id))) {
			terminal.warning(
				`createFavorite.service.js | Service ${service_id} not found or deleted.`,
			);
			return -4;
		}
		...
		if (existing) {
			terminal.warning(
				`createFavorite.service.js | Favorite already exists for customer ${customer_id} and service ${service_id}.`,
			);
			return -6;
		}
	}

...
exports.uploadImage = async (fileName, file) => {
    try {
        ...
        terminal.success(`uploadMedia.service.js -> uploadImage | Image uploaded: ${result.secure_url}`);
...
exports.uploadImages = async (folderName, files) => {
    try {
        ...
        const folderUrl = results.length > 0 ? results[0].replace(/\/image_1\..+$/, '') : null;
        terminal.success(`uploadMedia.service.js -> uploadImages | Folder uploaded: ${folderUrl}`);

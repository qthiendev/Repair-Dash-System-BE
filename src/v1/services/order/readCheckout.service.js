...
module.exports = async (customer_id, service_id) => {
	...
	if (!customer) {
		terminal.warning(
			`readCheckout.service.js | Customer ${customer_id} not found.`,
		);
		return -1;
	}
	...
	if (!service) {
		terminal.warning(
			`readCheckout.service.js | Service ${service_id} not found.`,
		);
		return -2;
	}
	...
	if (!service.owner) {
		terminal.warning(
			`readCheckout.service.js | Store not found for service ${service_id}.`,
		);
		return -3;
	}
	...
	if (service.owner_id === customer_id) {
		terminal.warning(
			`readCheckout.service.js | Store ${service.owner_id} (customer ${customer_id}) cannot order its own service ${service_id}.`,
		);
		return -4;
	}

...
module.exports = async (
	...
	if (!customer) {
		terminal.warning(
			`order.service.js | Customer ${customer_id} not found.`,
		);
		return -1;
	}
	...
	if (!service.owner) {
		terminal.warning(
			`order.service.js | Store not found for service ${service_id}.`,
		);
		return -3;
	}
	...
	if (service.owner_id === customer_id) {
		terminal.warning(
			`order.service.js | Store ${service.owner_id} (customer ${customer_id}) cannot order its own service ${service_id}.`,
		);
		return -4;
	}

	// Check if the store has reached the order pending limit
	const ORDER_PENDING_LIMIT = 10;

	const orderPendingCount = await Order.count({
		include: [
			{
				model: Service,
				as: "service",
	...
	if (orderPendingCount >= ORDER_PENDING_LIMIT) {
		terminal.warning(
			`order.service.js | Order pending limit reached: ${ORDER_PENDING_LIMIT}.`,
		);
		return -5;
	}

	// Check if the store has reached the total order limit based on the plan
	const TOTAL_ORDER_PER_MONTH_BASIC = 20;
	const TOTAL_ORDER_PER_MONTH_MONTHLY = 100;
	const TOTAL_ORDER_PER_MONTH_YEARLY = 99999;
	let TOTAL_ORDER_PER_MONTH = null;

	if (service.owner.user_priority == 0)
		TOTAL_ORDER_PER_MONTH = TOTAL_ORDER_PER_MONTH_BASIC;
	else if (service.owner.user_priority == 1)
		TOTAL_ORDER_PER_MONTH = TOTAL_ORDER_PER_MONTH_MONTHLY;
	else if (service.owner.user_priority == 2)
		TOTAL_ORDER_PER_MONTH = TOTAL_ORDER_PER_MONTH_YEARLY;
	else return -5;
		...
		where: {
			service_id: {
				[Op.in]: Sequelize.literal(
					`(SELECT service_id FROM services WHERE owner_id = ${service.owner_id})`,
				),
			},
			created_at: {
	...
	if (orderCount >= TOTAL_ORDER_PER_MONTH) {
		terminal.warning(
			`order.service.js | Store owner ${service.owner_id} has reached the order limit this month.`,
		);
		return -5;
	}
	...
	if (folderUrl) {
		await Order.update(
			{ order_images_url: folderUrl },
			{ where: { order_id: newOrder.order_id } },
		);
	}

	await createNotification(customer_id, {
		type: "ORDER_CREATED",
		content: `Đơn của bạn đã được tạo thành công cho dịch vụ "${service_name}".`,
		link: `${newOrder.order_id}`,
	});

	await createNotification(service.owner_id, {
		type: "ORDER_CREATED",
		content: `Bạn có đơn hàng mới từ khách hàng "${customer_full_name}" cho dịch vụ "${service_name}".`,
		link: `${newOrder.order_id}`,
	});

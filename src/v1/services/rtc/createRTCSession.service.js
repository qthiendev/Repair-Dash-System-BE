...
module.exports = async (order_id) => {
	...
	if (order[0] === 0) {
		terminal.warning(
			`createRTCSession.service.js | RTC session ${session_id} not created for order ${order_id}.`,
		);
		return -1;
	}

	terminal.success(
		`createRTCSession.service.js | RTC session ${session_id} created for order ${order_id}.`,
	);
	return sessionData;
};

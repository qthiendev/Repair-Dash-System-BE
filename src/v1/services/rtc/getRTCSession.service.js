...
module.exports = async (user_id, session_id) => {
    ...
    if (!order) {
        terminal.warning(`getRTCSession.service.js | RTC session ${session_id} not found or unauthorized for user ${user_id}.`);
        return -1;
    }
    ...
    const formattedMessages = messages.map(({ value, score }) => {
        ...
        return { user_id: senderId, is_sender: senderId === user_id, message, timestamp: realTime };
    });

...
module.exports = async (user_id, session_id, message) => {
    ...
    if (!order) {
        terminal.warning(`updateRTCSession.service.js | RTC session ${session_id} not found or unauthorized for user ${user_id}.`);
        return -1;
    }
    ...
    const messageData = JSON.stringify({ senderId: user_id, message, timestamp });

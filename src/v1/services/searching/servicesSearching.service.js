...
const calculateServicePriority = async (service, normInput) => {
	...
	const serviceDescription = normalizeString(
	...
	priority += similarityScore(
		normDistrict,
		ownerDistrict,
		LOCATION_WEIGHTS.district,
	);
	priority += similarityScore(normWard, ownerWard, LOCATION_WEIGHTS.ward);
	priority += similarityScore(
		normStreet,
		ownerStreet,
		LOCATION_WEIGHTS.street,
	);
	...
	if (normKeyword) {
		if (serviceName === normKeyword)
			priority += KEYWORD_WEIGHTS.exact_service_name;
		else
			priority += similarityScore(
				normKeyword,
				serviceName,
				KEYWORD_WEIGHTS.partial_service_name,
			);

		if (ownerName === normKeyword)
			priority += KEYWORD_WEIGHTS.exact_owner_name;
		else
			priority += similarityScore(
				normKeyword,
				ownerName,
				KEYWORD_WEIGHTS.partial_owner_name,
			);

		priority += similarityScore(
			normKeyword,
			serviceDescription,
			KEYWORD_WEIGHTS.description_match,
		);
	}
	...
	try {
		...
		const servicesWithPriority = await Promise.all(
			services.map((service) =>
				calculateServicePriority(service, normInput),
			),
		);

		const sortedServices = servicesWithPriority.sort(

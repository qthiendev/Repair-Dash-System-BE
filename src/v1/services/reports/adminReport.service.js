...
module.exports = async (index = 1, max_range = 20, filters = {}) => {
	...
	if (filters.user_full_name)
		userWhere.user_full_name = { [Op.like]: `%${filters.user_full_name}%` };
	if (filters.user_alias)
		userWhere.user_alias = { [Op.like]: `%${filters.user_alias}%` };
	if (filters.user_phone_number)
		userWhere.user_phone_number = {
			[Op.like]: `%${filters.user_phone_number}%`,
		};

	const authWhere = { delete_flag: false };
	if (filters.role) authWhere.role = filters.role;
	if (filters.identifier_email)
		authWhere.identifier_email = {
			[Op.like]: `%${filters.identifier_email}%`,
		};
	...
	terminal.success(
		`getUserStatistics.service.js | Retrieved user statistics: ${totalUsers} total users, ${storeUsers} store users, ${customerUsers} customer users, ${adminUsers} admin users, page ${index} of ${total_pages}, filters: ${JSON.stringify(filters)}`,
	);

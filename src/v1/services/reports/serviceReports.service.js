...
module.exports = async (
	...
	terminal.success(
		`readServiceReport.service.js | Found ${services.length} services for user ${user_id}, service_id: ${service_id || "all"}`,
	);
	...
	const serviceReport = services.map((service) => {
		const report = {
			...
			total_canceled_orders:
				Number(service.dataValues.total_canceled_orders) || 0,
			total_completed_orders:
				Number(service.dataValues.total_completed_orders) || 0,
			total_pending_orders:
				Number(service.dataValues.total_pending_orders) || 0,
			total_processing_orders:
				Number(service.dataValues.total_processing_orders) || 0,
		...
		terminal.success(
			`readServiceReport.service.js | Service ${service.service_id}`,
		);
		return report;
	});
	...
	const total = serviceReport.reduce(
		(acc, curr) => ({
			...
			total_canceled_orders:
				acc.total_canceled_orders + curr.total_canceled_orders,
			total_completed_orders:
				acc.total_completed_orders + curr.total_completed_orders,
			total_pending_orders:
				acc.total_pending_orders + curr.total_pending_orders,
			total_processing_orders:
				acc.total_processing_orders + curr.total_processing_orders,
	...
	if (service_id) {
		...
		return {
			...
			service: {
				...
				total_completed_orders:
					singleReport.total_completed_orders || 0,
				total_pending_orders: singleReport.total_pending_orders || 0,
				total_processing_orders:
					singleReport.total_processing_orders || 0,
	...
	terminal.debug(
		`${startIndex} - ${endIndex} | ${total_services} | ${max_range} | ${total_pages}`,
	);
	return {
		total_pages: total_pages,
		current_page: index,

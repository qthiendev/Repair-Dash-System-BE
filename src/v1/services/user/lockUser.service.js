...
module.exports = async (user_id) => {
    ...
    try {
        ...
        if (updatedRows === 0) {
            await transaction.rollback();
            terminal.error(`lockUser.service.js | Failed to lock user ${user_id}`);
            return false;
        }

        await transaction.commit();
        terminal.success(`lockUser.service.js | Successfully ${(user.delete_flag ? 'unlocked' : 'locked')} user ${user_id}`);
        return user.delete_flag ? 1 : 2;
    } catch (error) {
        await transaction.rollback();
        terminal.error(`lockUser.service.js | Error locking user ${user_id}: ${error.message}`);
        return false;
    }
};

...
module.exports = async (user_id) => {
	...
	try {
		...
		if (!user) {
			...
			terminal.info(
				`deleteUser.service.js | User ${user_id} not found or already soft-deleted`,
			);
			return -1;
		}
		...
		if (userUpdatedRows === 0 || authUpdatedRows === 0) {
			await transaction.rollback();
			terminal.error(
				`deleteUser.service.js | Failed to update User or Authentication for user ${user_id}`,
			);
			return false;
		}
		...
		terminal.success(
			`deleteUser.service.js | Successfully soft-deleted user ${user_id} and associated authentication`,
		);
		return true;
	} catch (error) {
		await transaction.rollback();
		terminal.error(
			`deleteUser.service.js | Error soft-deleting user ${user_id}: ${error.message}`,
		);
		return false;
	}
};

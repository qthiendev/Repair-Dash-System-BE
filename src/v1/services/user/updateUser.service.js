...
module.exports = async (user_id, updateData) => {
    ...
    try {
        ...
        if (!user) {
            ...
            terminal.warning(`updateUser.service.js | User ${user_id} not found or already soft-deleted`);
            return -1;
        }

        if (updateData.user?.user_alias && updateData.user.user_alias !== user.user_alias) {
            ...
            if (existingAlias) {
                await transaction.rollback();
                terminal.warning(`updateUser.service.js | Alias ${updateData.user.user_alias} already taken for user ${user_id}`);
                return -2;
            }
        }
        ...
        if (updateData.user.avatar_image) {
            ...
            const avatarUrl = await uploadService.uploadImage(fileName, updateData.user.avatar_image);
            if (!avatarUrl) {
                await transaction.rollback();
                terminal.error(`updateUser.service.js | Error uploading avatar for user ${user_id}`);
                return -3;
            }
        ...
        if (authUpdateData.password) {
            authUpdateData.password = await bcrypt.hash(authUpdateData.password, SALT_ROUNDS);
        }
        ...
        const [authUpdatedRows] = await Authentication.update(
            authUpdateData,
            { where: { authentication_id: user.authentication_id }, transaction }
        );

        if (userUpdatedRows === 0 && authUpdatedRows === 0) {
            await transaction.rollback();
            terminal.error(`updateUser.service.js | No updates applied for user ${user_id}`);
            return -3;
        }
        ...
        if (!updatedUser) {
            await transaction.rollback();
            terminal.error(`updateUser.service.js | Updated user ${user_id} not found after update`);
            return -3;
        }

        await transaction.commit();
        terminal.success(`updateUser.service.js | User ${user_id} and authentication updated successfully`);
        ...
        await transaction.rollback();
        terminal.error(`updateUser.service.js | Error updating user ${user_id}: ${error.message}`);
        return -3;
    }
};

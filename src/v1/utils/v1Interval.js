...
const { Order, Payment, User, Service } = require("../models/index.model");
const createNotification = require("../services/notification/createNotification.service");

const FIFTEEN_MINUTES = 115 * 60 * 1000;
...
const checkPendingOrders = async () => {
	try {
		...
		const pendingOrdersOvertime = pendingOrders.filter(
			(order) => new Date(order.created_at) <= fifteenMinutesAgo,
		);
		...
		if (pendingOrdersOvertime.length === 0) {
			terminal.info(
				`checkPendingOrders | Current pending orders: ${pendingOrders.length}, there is no order overtime`,
			);
		} else {
			terminal.info(
				`checkPendingOrders | Current pending orders: ${pendingOrders.length}, ${pendingOrdersOvertime.length} order overtime`,
			);
		}
		...
		for (const order of pendingOrdersOvertime) {
			...
			try {
				await createNotification(order.service.owner_id, {
					type: "ORDER_AUTO_CANCELED",
					content: `Hệ thống đã tự động hủy đơn hàng #${order.order_id} do quá thời gian chờ.`,
					link: `${order.order_id}`,
				});
				...
				terminal.error(
					`checkPendingOrders | Error creating notification: ${error.message}`,
				);
			}

			terminal.info(
				`checkPendingOrders | Order ${order.order_id} canceled (created at ${order.created_at})`,
			);
		}
	} catch (error) {
...
const checkSubscriptions = async () => {
	try {
		...
		if (activePayments.length === 0) {
			terminal.info(
				"checkSubscriptions | No active subscriptions to check",
			);
			return;
		} else {
			terminal.info(
				`checkSubscriptions | Current active subcription: ${activePayments.length}`,
			);
		}
		...
		for (const payment of activePayments) {
			...
			if (now > expirationDate && payment.User.user_priority !== 0) {
				...
				terminal.info(
					`checkSubscriptions | User ${payment.User.user_id} priority reset to 0 (subscription expired)`,
				);
			}
		}
	} catch (error) {
...
const intervalController = {
	 ...
	 */
	start: function () {
		this.intervals.order = runtimeInterval(
			checkPendingOrders,
			FIVE_MINUTES,
			"PendingOrderCheck",
		);
		this.intervals.subscription = runtimeInterval(
			checkSubscriptions,
			ONE_HOUR,
			"SubscriptionCheck",
		);

...
const createServiceValidation = validateRequest([
	body("service_name")
		...
		.withMessage("Name service must be between 6 and 100 characters"),

	body("service_alias")
		.isString()
		.isLength({ min: 1, max: 50 })
		.withMessage("Alias must be between 1 and 50 characters"),

	body("service_description")
		.isLength({ min: 6, max: 500 })
		.withMessage(
			"Description service must be between 6 and 500 characters",
		),
]);
		...
		.custom((value) => {
			if (typeof value === "string") {
				...
				if (!Number.isInteger(value) || value <= 0) {
					throw new Error(
						"service ID must be a positive integer greater than 0",
					);
				}
			} else {
...
const readServiceStoreValidation = validateRequest([
	param("owner_id").custom((value) => {
		if (typeof value === "string") {
			...
			if (!Number.isInteger(value) || value <= 0) {
				throw new Error(
					"Owner ID must be a positive integer greater than 0",
				);
			}
		} else {

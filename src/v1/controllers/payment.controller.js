...
exports.createZaloPay = async (req, res) => {
    try {
        ...
        switch (result) {
            case -1:
                return res.status(400).json({ message: 'Invalid subscription type', code: -1 });
            case -2:
                return res.status(400).json({ message: 'User not found', code: -2 });
            case -3:
                return res.status(400).json({ message: 'Failed to create payment record', code: -3 });
            default:
                return res.status(200).json(result);
        }
    } catch (error) {
        terminal.error(`zalopay.controller.js | Create Payment Error: ${error.message}`);
...
exports.checkZaloPay = async (req, res) => {
    try {
        ...
        switch (result) {
            case -1:
                return res.status(400).json({ message: 'Transaction not found', code: -1 });
            case -2:
                return res.status(500).json({ message: 'Failed to get response from ZaloPay server', code: -2 });
            case -3:
                return res.status(500).json({ message: 'Failed to update payment status', code: -3 });
            default:
                return res.status(200).json(result);
        }
    } catch (error) {
        terminal.error(`zalopay.controller.js | Check Payment Error: ${error.message}`);
        return res.status(500).json({ message: 'Unexpected error occurred' });
    }
};
...
exports.getSubscription = async (req, res) => {
    try {
        ...
        if (result === -1) {
            return res.status(400).json({ message: 'Payment not found', code: -1 });
        }

        return res.status(200).json(result);
    } catch (error) {
        terminal.error(`zalopay.controller.js | Get Subscription Error: ${error.message}`);
        return res.status(500).json({ message: 'Unexpected error occurred' });
    }  
};
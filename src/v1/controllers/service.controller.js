...
exports.createService = async (req, res) => {
  try {
    const owner_id = jwt.verify(req.cookies?.accessToken,process.env.JWT_SECRET_KEY).user_id;

    const { service_name, service_description } = req.body;

    const serviceId = await createService(owner_id,service_name,service_description);

    switch (serviceId) {
      case -1:
        return res.status(400).json({ message: "Owner not found.", code: -1 });
      case -2:
        return res.status(400).json({ message: "Service already exists.", code: -2 });
      default:
...
exports.updateService = async (req, res) => {
  try {
    ...
    const result = await updateService(owner_id,service_id,service_name,service_description);

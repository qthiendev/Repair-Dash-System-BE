...
exports.readCheckout = async (req, res) => {
    try {
        const result = await readCheckoutService(
            await jwt.verify(req.cookies?.accessToken, process.env.JWT_SECRET_KEY).user_id,
        ...
        switch (result) {
            ...
            case -2:
                return res.status(404).json({ message: 'Service not found.', code: -2 });
            case -3:
                return res.status(404).json({ message: 'Store not found for this service.', code: -3 });
            case -4:
                return res.status(403).json({ message: 'Store not allow to order own service.', code: -4 });
            default:
                return res.status(200).json({ checkout: result });
        }
    } catch (error) {
        terminal.error(`order.controller.js | Read Checkout Error: ${error.message}`);
...
exports.createOrder = async (req, res) => {
    try {
        const customer_id = jwt.verify(req.cookies?.accessToken, process.env.JWT_SECRET_KEY).user_id;
        const {
            service_id,
            order_description,
            customer_full_name,
        ...
        switch (order_id) {
            case -1:
                return res.status(400).json({ message: 'Customer not found.', code: -1 });
            case -2:
                return res.status(400).json({ message: 'Service not found.', code: -2 });
            case -3:
        ...
        terminal.error(`order.controller.js | Create Order Error: ${error.message}`);
        res.status(500).json({ message: 'Unexpected error occurred' });
    }
};
...
exports.updateOrder = async (req, res) => {
    try {
        ...
        switch (result) {
            case -1:
                return res.status(404).json({ message: 'Order not found', code: -1 });
            case -2:
                return res.status(400).json({ message: 'Employee not found', code: -2 });
            case -3:
                return res.status(403).json({ message: 'You are not allowed to update this order', code: -3 });
            case -4:
                return res.status(403).json({ message: 'Unauthorized to change this order', code: -4 });
            case -5:
                return res.status(400).json({ message: 'Order is COMPLETED, only order_feedback can be updated', code: -5 });
            case -6:
                return res.status(400).json({ message: 'Employee does not belong to this service owner.', code: -6 });
            default:
                return res.status(200).json({ message: 'Order updated successfully', order: result });
        }
    } catch (error) {
        terminal.error(`order.controller.js | Update Order Error: ${error.message}`);
        res.status(500).json({ message: 'Unexpected error occurred' });
    }
};
...
exports.readOrder = async (req, res) => {
    try {
        const { user_id } = jwt.verify(req.cookies?.accessToken, process.env.JWT_SECRET_KEY);
        const { order_id } = req.params;
    ...
    try {
        ...
        switch (result) {
            case -1:
                return res.status(404).json({ message: 'Order not found', code: -1 });
            case -2:
                return res.status(400).json({ message: 'Order is already deleted', code: -2 });
            case -3:
                return res.status(403).json({ message: 'You are not allowed to delete this order', code: -3 });
            default:
                return res.status(200).json({ message: 'Order deleted successfully' });
        }
    } catch (error) {
        terminal.error(`order.controller.js | Delete Order Error: ${error.message}`);
        res.status(500).json({ message: 'Unexpected error occurred' });
    }
};

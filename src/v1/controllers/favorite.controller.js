...
exports.getFavorites = async (req, res) => {
	try {
		...
		terminal.error(
			`favorite.controller.js | Error retrieving favorites: ${error.message}`,
		);
...
exports.createFavorite = async (req, res) => {
	try {
		...
		const result = await createFavoriteService(
			user_id,
			store_id,
			service_id,
		);
		...
		switch (result) {
			...
			case -2:
				return res.status(400).json({
					message:
						"Must provide either store_id or service_id, but not both",
					code: -2,
				});
			case -3:
				return res
					.status(404)
					.json({ message: "Store not found or deleted", code: -3 });
			case -4:
			...
			case -5:
				return res
					.status(400)
					.json({ message: "User is not a store", code: -5 });
			case -6:
				return res
					.status(400)
					.json({ message: "Favorite already exists", code: -6 });
			default:
				return res.status(201).json({ favorite_id: result });
		}
	} catch (error) {
		terminal.error(
			`favorite.controller.js | Error adding favorite: ${error.message}`,
		);
		res.status(500).json({ message: "Unexpected error occurred" });
	}
};
...
exports.deleteFavorite = async (req, res) => {
	try {
		...
		terminal.error(
			`favorite.controller.js | Error deleting favorite: ${error.message}`,
		);
		res.status(500).json({ message: "Unexpected error occurred" });
	}
};
